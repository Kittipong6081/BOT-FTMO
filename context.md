...
> Last Updated: 2026-05-30 (v8.1-phase3 — TF 3-brain training pipeline; code ready to train, TF still paper) | Scope: Hub / Index — read this first, then drill into wiki/*

## TL;DR (LLM read first — 30-second scan)

- **v8.1-phase3 (2026-05-30) — 🏗️ TF training pipeline (3-brain, mirror of MR; code ready, not yet trained → TF still paper)**: NEW `ml/trend_following_backtester.py` (`TrendFollowingBacktester`, entry H1, wide-RR resolve with runners, D1 resampled from H4), `ml/trend_following_env.py` (`TrendFollowingFilterEnv`, obs tf_v1 = 35-dim 3 slots reinterpreted, reward INVERTED: reward runners ≥2R / penalize late entry), scripts `build_tf_signal_pool` / `train_tf_signal_quality` (→`data/tf_signal_quality_model.pkl`) / `train_tf_signal_filter` (→`models/tf/`) / `auto_train_pipeline_tf` (slim). `TrendFollowingStrategy`: resuming+MACD → SOFT confluence (strict conjunction gave ~0 signals). main: `_rl_agents`/`_quality_models` dicts + `_build_obs_tf` route by strategy_id; **TF executes only when `paper_mode=False AND models/tf loaded`**. ✅ full pipeline runs end-to-end (60-ep smoke: build→GBM→PPO 2-phase+eval); leakage+parity exit 0 (MR obs 35 intact). **⚠️ 2 obs layouts now** (mr_v8 + tf_v1). ดู [`wiki/05-invariants.md` v8.1-phase3](wiki/05-invariants.md#-version-log-entry--v81-phase3-2026-05-30--tf-training-pipeline-3-brain-backtester--env--scripts--agent-wiring).
- **v8.1-phase2 (2026-05-30) — 🧯 Per-strategy magic + Order Conflict Filter + StrategyRiskBook (bot_state schema 9; MR unchanged when flag OFF)**: Phase 2 ของ dual-strategy — สร้างเครื่องจักรฝั่ง execution+risk. **ทุกอย่างกั้นด้วย `bot_config.tf.enabled` (default OFF) → MR เดิมเป๊ะ**; TF ยัง paper จนถึง Phase 3. (1) per-strategy magic (MR 123456 / TF 123457) → `ExecutedTrade.strategy_id` + `TrailingState.strategy_id`, orphan recovery ใช้ `_bot_magics()` (เดิม hardcode 123456). (2) NEW `TradeExecutor._check_strategy_conflict` (regime-exclusivity + slot cap MR 2/TF 1) เป็นด่านแรก; `_check_entry_confirmation` กลายเป็น MR-only (TF ข้าม). (3) NEW `core/strategy_risk_book.py` `StrategyRiskBook` — แยกงบ MR 2.0%/TF 1.5% (realized@close + floating live จาก magic), `is_halted(sid)` halt เฉพาะตัวไม่แตะ global BotState; `can_open_trade(strategy_id=)` + schema 9 + soft cap 3.0% เฉพาะ dual. ✅ flag OFF: leakage+parity exit 0 (risk 0.70% = training); unit test ledger/conflict/magic/cap ผ่าน. ดู [`wiki/05-invariants.md` v8.1-phase2](wiki/05-invariants.md#-version-log-entry--v81-phase2-2026-05-30--per-strategy-magic--order-conflict-filter--strategyriskbook-bot_state-schema-9).
- **v8.1-phase1 (2026-05-30) — 🧭 Market Regime gate + StrategyRouter + Trend Following scanner (rules-only, paper-mode, flag OFF default)**: Phase 1 ของแผน dual-strategy. **Master switch `bot_config.tf.enabled` default False → live = MR เดิมเป๊ะ** (router=None). เปิด flag → router จำแนก regime ต่อคู่แล้ว arm MR (RANGING) / TF (TRENDING) / ไม่เทรด (AMBIGUOUS, ADX 20-27 = killer zone v8.0.51). NEW `MarketRegimeClassifier` (ADX H1 + choppiness + slope + hysteresis), `StrategyRouter`, `TrendFollowingStrategy/Scanner` + `TFSignal` (entry H1, trend H4/D1, SL=ATR×2.0, RR 2.5 wide, magic 123457), `TechnicalIndicators.calculate_choppiness`, config `bot_config.tf` + `bot_config.regime`. TF = **paper_mode (log `TF_PAPER`, ยังไม่ยิงจริง)** จนถึง Phase 2. ✅ flag OFF: leakage+parity exit 0 (obs 35, MR identical); flag ON: functional test ผ่าน (dead-zone/hysteresis/router/lock/TF geometry). ดู [`wiki/05-invariants.md` v8.1-phase1](wiki/05-invariants.md#-version-log-entry--v81-phase1-2026-05-30--market-regime-gate--strategyrouter--trend-following-scanner-rules-only-paper-mode-flag-off-by-default).
- **v8.1-phase0 (2026-05-30) — 🧩 Strategy abstraction (groundwork for parallel Trend Following; MR behaviour UNCHANGED)**: NEW `strategy/strategy_base.py` `StrategyBase` (ABC: `STRATEGY_ID`/`MAGIC_NUMBER`/`OBS_LAYOUT_ID` + `scan_all_symbols(allowed_symbols=None)` + `get_{ltf,mtf,htf}_data`); `LiveMRScanner(StrategyBase)` (`STRATEGY_ID="MR"`, magic 123456); `MRSignal.strategy_id="MR"` (field ใหม่); `main.py` `_strategies` registry + `_strategy_for(sig)` → route obs/context ตาม signal ที่ผลิต (กัน cross-strategy contamination, ต่อยอด v8.0.79). ✅ leakage + parity audit ผ่าน (obs dim ยัง 35), env/obs/model ไม่แตะ → MR identical. ดู [`wiki/05-invariants.md` v8.1-phase0](wiki/05-invariants.md#-version-log-entry--v81-phase0-2026-05-30--strategy-abstraction-groundwork-for-parallel-trend-following).

- **v8.0.81 (2026-05-30) — 🔬 Per-dim obs parity tool + ADX-penalty reward fix (RL retrain, PROMOTED)**: (1) NEW `scripts/obs_parity_check.py` replays real pool signals through train (`MeanReversionFilterEnv._get_obs`) + live (`_build_signal_observation`) and diffs per-dim → **proved all 23 signal-derived obs dims byte-identical (Δ=0)** (H3/H4 confirmed; obs[31] mins_since uses identical session formula = replay artifact, not a bug). ⛔ run it after any `_get_obs`/`_build_signal_observation` change. (2) Removed `MeanReversionFilterEnv` **ADX-violation penalty** — it punished M15 ADX>25 but the real veto is H1 ADX>30 (hard-gated at pool-build, all pool signals ≤30) → it penalized ~half of valid MR signals. RL-only retrain (pool/GBM unchanged). **Result: holdout 89.0% > v8.0.80 87.6% (+1.4pp)**, Take 44.8→47.2%, Win 69.5% flat, Δ −2.1pp HEALTHY → promoted. ดู [`wiki/05-invariants.md` v8.0.81](wiki/05-invariants.md#-version-log-entry--v8081-2026-05-30--per-dim-obs-parity-tool--adx-penalty-reward-fix-follow-up-to-v8080).

- **v8.0.80 (2026-05-30) — 🛡️ Production source-code audit remediation (109 findings → 13 confirmed; RETRAIN for C3/H6)**: เพ่งเล็ง cache contamination / dead logic / execution edge cases / clean code. **🔴 Critical**: (C1) `TradeManager._modify_sl` ไม่เช็ค broker min-stop/freeze → BE@0.3R/Stage-2 lock@0.5R โดน reject เงียบ ๆ → SL ค้าง full-risk; แก้ clamp ระยะ + กันถอยหลัง (อ่าน SL จริงจาก broker) + log retcode + retry transient. (C2) `RiskManager.check_risk` เพิ่ม **always-on FTMO 4% breach guard** — เดิม `_check_daily_loss` early-return ตอน `DAILY_HALT` (ที่ตั้งโดย consec-loss/stop-out แบบไม่ปิดไม้) → ไม้ค้าง bleed เกิน 4% โดยไม่ปิดฉุกเฉิน. (C3) `MeanReversionFilterEnv.step` ตกด่าน entry-confirm forced-SKIP ที่ parent+live มี → agent ฝึก TAKE บน ~33% ที่ live block (train≠live) → **ต้อง retrain**. **🟠 High**: H1 retcode log+retry; H2 per-symbol trade counter (`_daily_trades_by_symbol`); H3 obs[16] ใช้ ml_score ที่มี temporal (live=train); H4 obs[21] นับไม้เปิดสะสมวันนี้ (=training); H5 re-anchor SL/TP ตอนยิง + INVALID_STOPS; H6 pool builder anchor HTF ต่อ scan-bar (**rebuild pool**); H7 `auto_train_pipeline --ml_threshold` 0.40→0.30; H8 `parity_audit` อ่าน argparse default. **🟡🟢**: ลบ single-slot cache, counter `%720`→wall-clock, `is_connected` total_seconds, close deviation per-symbol, docstring เก่า. **✅ Refuted 7** (min-stop ตอนเปิด, race, ticket≠position-id, tz, ฯลฯ — ปลอดภัยแล้ว). Live-only fixes deploy ได้เลย; C3/H6 ต้อง retrain (backups `*.pre_v8080`, holdout gate ≤10pp). ดู [`wiki/05-invariants.md` v8.0.80](wiki/05-invariants.md#-version-log-entry--v8080-2026-05-30--production-audit-remediation-execution-safety--ftmo-breach-guard--trainlive-parity-retrain-required-for-c3h6).

- **v8.0.79 (2026-05-29) — 🐛 Cross-symbol cache contamination fix + currency-leg correlation cap (live-only, no retrain)**: RCA log 29 พ.ค. (แพ้ 5/5 = −$219.63) พบ 2 ปัญหาโครงสร้าง. **(1) Cache contamination**: `LiveMRScanner` เก็บ `_ltf_data`/`_mtf_data`/`_htf_data` เป็น slot เดียวที่ถูกเขียนทับทุก symbol ใน `scan_all_symbols` → `FTMOTradingBot._build_live_context` (เรียกหลังสแกนครบ) อ่านค่าของ **คู่สุดท้ายที่สแกน** ไม่ใช่ของ sig.symbol. หลักฐาน: ADX H1 `43.17125997031108` ถูก log ให้ทั้ง GBPJPY+EURJPY รอบเดียวกัน. กระทบ (a) คอลัมน์ ADX H1/H4 ใน Excel เชื่อไม่ได้ (corrupt diagnostics — รวมการวิเคราะห์เก่าที่อิงคอลัมน์นี้) (b) `compute_temporal_features` คำนวณ `atr_zscore_30bars`+`volatility_regime_score` (GBM/RL obs) จากคู่ผิด. ⚠️ trend filter **ไม่ได้พัง** (ADX จริงในด่านกรอง 16–20 = ranging, ผ่านถูก). Fix: per-symbol cache `LiveMRScanner._{ltf,mtf,htf}_by_symbol` + accessors `get_{ltf,mtf,htf}_data(symbol)`; `_build_live_context` อ่านตาม `sig.symbol`. **(2) Currency-leg cap**: EURUSD SELL + EURJPY SELL (short EUR ทั้งคู่) หลุดเพราะ `MAX_CORRELATED_POSITIONS=99` (group guard ปิด) + USD-theme cap ครอบแค่ leg USD. เพิ่ม `TradeExecutor._non_usd_legs` + `MAX_SAME_CURRENCY_LEG_POSITIONS=1` ใน `_check_correlation_risk` → กันซ้อนทิศบน leg ที่ไม่ใช่ USD (EUR/GBP/JPY/AUD/NZD/CHF/CAD/XAU). **Live-only** — obs dims เท่าเดิม, การแก้ทำให้ live ตรง training ขึ้น (parity ผ่าน, ดีขึ้น). ⚙️ ต้องยืนยัน VPS รัน commit ปัจจุบัน (local state ค้าง 22 พ.ค.). ดู [`wiki/05-invariants.md` v8.0.79](wiki/05-invariants.md#-version-log-reverse-chronological).
- **v8.0.78 (2026-05-29) — 📰 News close-window decoupled from entry-window (live-only, no retrain)**: User เจอไม้เปิดได้ 4 นาทีก็โดน Pre-news close (GBPJPY 29 พ.ค. 10:15→10:20, −$8.18). Root cause: ด่านเปิด (`can_open_trade` news block) กับด่านปิด (`TradeManager.check_news_close`) ใช้ `no_trade_before_news_minutes=60` **เลขเดียวกัน** → ไม้ที่เปิดช่วง 60-90 นาทีก่อนข่าว (ผ่านด่านเปิดเพราะ >60) โดนบังคับปิดทันทีตอนข่าวเข้า T-60 (runway=0) + ตัดไม้ที่กำลังทำงานทิ้งก่อนเวลา 1 ชม. (log: 2 ไม้ปิด T-60 ที่กำไรจิ๋ว +$12-13). Fix: เพิ่ม `SessionConfig.news_close_before_minutes=10` แยกด่านปิดออกจากด่านเปิด → entry คง 60 (กัน NFP aftermath ตาม v8.0.38), close ลดเป็น 10 → **runway การันตี ≥50 นาที** ทุกไม้ (ปิด T-10 ก็ flat ทันก่อน spike). เพิ่ม after-window 45→20 (กลับมาเทรดเร็วขึ้น). Bonus fix: `TradeExecutor.close_trade` คำนวณ `time_in_trade` (เดิมโชว์ 0 สำหรับไม้ปิดผ่าน close_trade เช่น Pre-news/Friday). Affected: `SessionConfig` (1 key NEW + 1 changed), `TradeManager.check_news_close`, `TradeExecutor.close_trade`. ดู [`wiki/05-invariants.md` v8.0.78](wiki/05-invariants.md#-version-log-reverse-chronological).
- **v8.0.77 (2026-05-29) — 🔓 Entry-confirm de-restriction (live-only, no retrain)**: Audit funnel จริง 953 scan (24-28 พ.ค.) พบ RL อยาก TAKE 689 ไม้ แต่ executor ปล่อยผ่านแค่ 23 (**3.3%**) — คอขวดอยู่ชั้น entry-confirm ไม่ใช่ตัวกลยุทธ์. แก้ 3 จุด live-only: **(1) M1 direction match (RF-1, 82/142 entry-confirm rejects = นักฆ่าอันดับ 1)** — เดิมบังคับแท่ง M1 ปิดตรงทิศ = สวนหลักการ MR "รับมีดที่กำลังตก" → เปลี่ยนเป็น **wick-aware**: ผ่านถ้า body ตรงทิศ *หรือ* มี rejection wick ≥ `ENTRY_CONFIRM_M1_REJECT_WICK_MIN=0.40`. **(2) KC distance (RF-2)** `KC_DIST_THRESHOLD_BASE` 0.60→**0.35** + `KC_DIST_ATR_SCALE_MAX` 1.5→**1.3** — เดิม volatile (0.90) ชน clip ceiling 1.0 เหลือ window 0.10 = "เกือบเป็นด่านตาย" + กรองซ้ำ BB %B; ใหม่ window 0.545-0.79. **(3) KC slope (RF-3)** `KC_SLOPE_THRESHOLD` 0.15→**0.35** — เดิม 0.15 ตัด EMA drift แค่ 0.045 ATR/bar + อิ่มตัว ±1.0 เร็ว. rollback ได้บรรทัดเดียว + parity เทรน/จริงดีขึ้น (training pool ไม่มี KC hard gate). Affected: `FTMOConfig` (4 keys), `TradeExecutor._check_entry_confirmation`. ดู [`wiki/05-invariants.md` v8.0.77](wiki/05-invariants.md#-version-log-reverse-chronological).
- **v8.0.76 (2026-05-28) — 📐 Dynamic KC distance (ATR-adaptive, live-only, no retrain)**: คู่กับ v8.0.75 slope — v8.0.74 ใช้ fixed threshold `|kc_distance_norm| ≥ 0.60` → ตลาด Asian-quiet มักไม่ถึง 0.6 (ตกรถ swing เล็ก), ตลาด US/news ผ่าน 0.6 ตอนกลางทาง (เข้าก่อน true extreme → โดนลาก). Fix: ใช้ `MRSignal.atr_zscore_30bars` ปรับ threshold เป็น `base × clip(1 + atr_z × 0.20, 0.6, 1.5)` → quiet (atr_z=-2) thresh หด **0.36** (รับ shallow extreme), volatile (atr_z=+2) ขยาย **0.84** (รอ deep extreme), max **0.90** (atr_z≥+3). **ทิศทาง opposite slope** — slope volatile = looser (noise OK), distance volatile = stricter (need true extreme). Affected: `FTMOConfig` (5 keys NEW), `TradeExecutor._check_entry_confirmation` (KC dist block). **Live-only** — ไม่ต้อง retrain. Rollback 1-line: `KC_DIST_ATR_ADAPTIVE = False`. ดู [`wiki/05-invariants.md` v8.0.76](wiki/05-invariants.md#-version-log-entry--v8076-2026-05-28--dynamic-kc-distance-atr-adaptive-live-only).
...
| **Correlation caps** | USD-theme cap **2** (`MAX_USD_THEME_POSITIONS`), non-USD currency-leg cap **1** (v8.0.79 `MAX_SAME_CURRENCY_LEG_POSITIONS` — กัน double-bet EUR/GBP/JPY/metal). Group guard `MAX_CORRELATED_POSITIONS=99` (off) | `TradeExecutor._check_correlation_risk` / `_non_usd_legs` |
...
| **Obs dims (v8)** | **35** (index 0-34; chronos[27,28] hardwired 0; obs=35 verified by parity_audit 3-way sync) | `SelfLearningAgent.OBS_DIM` |
