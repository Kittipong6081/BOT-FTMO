...
>
> **Last Updated:** 2026-06-04 (Fix#2 — เปิด/ปิดสมอง TF (Trend-Following) ด้วยสวิตช์ env `BOT_TF_ENABLED`, ค่าเริ่มต้น = ปิด)
>
> **🟢 Fix#2 status — สวิตช์เปิดสมองที่ 2 (TF / ตามเทรนด์) แบบ env (สรุปภาษาคน)**: ปกติบอทใช้ **MR สมองเดียว** (ค่าเริ่มต้นปลอดภัย — VPS/บัญชีจริงไม่เปลี่ยน). ถ้าอยากลองสมอง TF เพิ่ม ให้สั่งรันแบบนี้:
> - `python main.py` → MR อย่างเดียว (เหมือนเดิม)
> - `BOT_TF_ENABLED=1 python main.py` → **เปิด TF ยิงออเดอร์จริงบนบัญชีที่ต่ออยู่** (demo หรือ จริง)
> - `BOT_TF_ENABLED=1 BOT_TF_PAPER=1 python main.py` → เปิด TF แบบ **บันทึกอย่างเดียว ไม่ยิงออเดอร์** (ดูเฉย ๆ)
>
> ตอนเปิด TF: บอทจะแบ่งงานตามสภาพตลาด — ตลาดออกข้าง→MR, ตลาดเทรนด์→TF (ทำให้ MR เทรดน้อยลงเพราะถูกจำกัดเฉพาะตลาดออกข้าง). **มีสวิตช์เดียวคือ `BOT_TF_ENABLED`** — TF จะ "เข้าไว" (จับเทรนด์ตอนเพิ่งเกิด) เสมอโดยอัตโนมัติ (อัปเดต 2026-06-04 ตัดสวิตช์ early/late ทิ้ง หลังไม้แรกบน demo เข้าสายแบบเก่า). ⚠️ **คำเตือนตรง ๆ**: TF ยัง **ไม่ผ่านการทดสอบเงินปลอม (paper-forward)** — ช่องว่างซ้อม↔จริงนี่แหละที่ทำบัญชีก่อนตาย (−9.87%). **แนะนำ: รัน `BOT_TF_ENABLED=1` บน demo ก่อน ≥2–4 สัปดาห์** แล้วค่อยเปิดบนบัญชีจริง. ⚠️ ถ้าเทรน TF ใหม่ ต้อง**ปิดแล้วเปิดบอทใหม่**ถึงจะโหลดโมเดลใหม่. ดู [`wiki/04-operations.md`](wiki/04-operations.md).
>
