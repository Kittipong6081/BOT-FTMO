...
>
> **🔧 rebuild/smc-v2 (2026-06-06)** — data-prep tooling: `scripts/fetch_mt5_data.py` extended with **D1** (SMC HTF bias) + **M5** (SMC LTF entry) timeframes (run on an MT5 box; `MetaTrader5` pkg is Windows-only). The live loop described below is the MR `main`, unchanged on `main`; the SMC live path is built on branch `rebuild/smc-v2` (P3). See [`05-invariants.md` SMC v2 kickoff](05-invariants.md#-version-log-entry--smc-v2-rebuild-kickoff-2026-06-06-branch-rebuildsmc-v2-p0p2-done).
>
