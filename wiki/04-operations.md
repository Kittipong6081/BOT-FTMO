...
> Last Updated: 2026-06-06 (🔧 rebuild/smc-v2: `fetch_mt5_data.py` now fetches D1+M5 for SMC MTF; this file still describes the MR `main` loop) | Scope: main loop, RiskManager state machine, FTMO rules, news, trading sessions, console quiet mode, live logging, TF enable switch
>
> **🔧 rebuild/smc-v2 (2026-06-06)** — data-prep: `scripts/fetch_mt5_data.py` fetches **D1+M5** (SMC MTF); `--timeframe` now accepts ALL / one TF / a comma list. One-shot full re-export (run on an MT5 box; `MetaTrader5` is Windows-only): `python scripts/fetch_mt5_data.py --years 5` (ALL TFs) — or SMC-only (skip the 3-month-capped M1): `python scripts/fetch_mt5_data.py --timeframe D1,M5,M15,H1,H4 --years 5`. The live loop below is the MR `main`; the SMC live path is on branch `rebuild/smc-v2`. See [`05-invariants.md` SMC v2 kickoff](05-invariants.md#-version-log-entry--smc-v2-rebuild-kickoff-2026-06-06-branch-rebuildsmc-v2-p0p2-done).
>
...
- **v8.0.55 pre-execution gates** (in addition to RiskManager + correlation):
  ...
  - `TradeExecutor._check_correlation_risk` — duplicate-symbol block; USD-theme cap `MAX_USD_THEME_POSITIONS (2)`; non-USD currency-leg cap `MAX_SAME_CURRENCY_LEG_POSITIONS` (**1** — set 1 in v8.0.79, raised to 2 on 2026-06-02, **reverted to 1 on 2026-06-03** after the challenge-fail audit found cap=2 enabled a 3-leg short-JPY BUY cluster in 16 min that all lost; back to no double-bet on a shared non-USD leg) via `_non_usd_legs`. Per-group guard `MAX_CORRELATED_POSITIONS=99` is off.
