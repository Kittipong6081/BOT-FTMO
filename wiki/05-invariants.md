...
> Last Updated: 2026-06-06 | Scope: red flags, version log, migration notes (latest: **SMC v2 rebuild KICKOFF on branch rebuild/smc-v2 — strict-SMC 3-brain, P0–P2 done**)

## 📝 Version Log Entry — SMC v2 rebuild KICKOFF (2026-06-06, branch `rebuild/smc-v2`, P0–P2 done)

**Context**: user decision (informed, after the prior history was surfaced) to RIP OUT the Mean-Reversion strategy and rebuild a **strict Smart Money Concepts (SMC) 3-brain** system on a NEW branch `rebuild/smc-v2` (branched from `main` = MR production, left UNTOUCHED = rollback). SMC was the original v6–v7 strategy: it lost money live (PF 0.96, −$1.51/trade, WR 46%, ~6% sim pass) and was deleted in `7a9fdb1` ("remove smc"). This round MUST beat old SMC, judged on the **realistic FTMO sim** (Fix#3 spread cost + live-exit + walk-forward OOS). Plan: `~/.claude/plans/role-senior-quant-velvet-blanket.md`. Old-SMC failure modes to NOT repeat: confluence-points soup (not strict sequential), wide RR vs low WR (−EV), D1 hard-veto cutting ~80% of signals, sim too optimistic.

**Done so far (P0–P2)**:
- **P0**: branch created; `scripts/fetch_mt5_data.py` extended with **D1 + M5** timeframes (SMC MTF needs D1 bias + M5 entry). D1/M5 CSVs must be fetched on an MT5 box — the `MetaTrader5` pkg does not run on the Mac.
- **P1a**: deleted **144** backup/snapshot artifacts (`*.bak_*`, `*.pre_*`, `*.late_entry`, `models/mr/best.fix1`+`best.pre_fix1`) + all `__pycache__`; `import main` still clean. Deletion of MR/TF/dual-strategy SOURCE is deferred to P3 (coupled with the `main.py` rewrite so imports never break mid-flight).
- **P2**: NEW `strategy/smc/` engine package (Brain 1 = rules) — 6 engines, all **no-lookahead / leakage-safe** (a swing is usable only from its `confirm_index = index + right`): `SwingEngine`, `StructureEngine` (BOS=continuation / CHOCH=reversal, body-close confirmed), `LiquidityEngine` (equal H/L, sweeps, dealing range premium/discount/OTE), `FVGEngine` (3-candle + mitigation), `OrderBlockEngine` (last opposing candle before a BOS + grade + mitigation), `BiasEngine` (HTF MTF bias + liquidity target). Tests `tests/test_smc_engines.py` **8/8 pass**.

- **P3.1 (done)**: `EntryEngine` strict state machine (`strategy/smc/entry.py`, HTF bias → discount/premium → LTF sweep → CHOCH/BOS → unmitigated OB/FVG zone → structural SL/TP + RR gate) + `SMCSignal`/`SMCSignalType` (`strategy/smc/types.py`) + `SMCScanner(StrategyBase)` (`strategy/smc_strategy.py`, magic **123458**, fetches D1/H4/M15). Tests `tests/test_smc_engines.py` now **9/9** (incl. end-to-end BUY setup). `import main` still clean (MR untouched).

- **P3.2 (done)**: `main.py` live path rewritten to SMC — `FTMOTradingBot` now builds `SMCScanner` (magic 123458), loads RL from `models/smc/` + GBM from `data/smc_signal_quality_model.pkl`, and `_build_signal_observation` emits the **smc_v1 24-dim** obs. ⛔ `SelfLearningAgent.OBS_DIM` 36→**24** (smc_v1 — must stay synced with `_build_signal_observation` + future `SMCFilterEnv._get_obs`; run `obs_parity_check` after changes). Dual-strategy router + per-strategy agent/GBM dicts + dead regime methods + Chronos removed. **DELETED** (branch): `trend_following_strategy/backtester/env`, `regime_classifier`, `strategy_router`, `chronos_forecaster`, + TF scripts + `diagnose_chronos`. `import main` + `FTMOTradingBot()` mock-init clean; SMC engine tests 9/9. **DEFERRED to P5**: deleting MR core (`mean_reversion_strategy/backtester/env` + MR scripts) — `ml/signal_filter_env.py` (base env) + audits still import them; they go when `ml/smc_env.py`/`ml/smc_backtester.py` replace them. `strategy_risk_book` KEPT (per-strategy ledger works for SMC).

- **P4 (done)**: `TradeManager` exit management is SMC. `_exit_profile` returns one SMC profile (`trail_mode="structure"`, partial on, tp_step off); the runner trails via NEW `_structure_trail` — re-fetches M15, runs `SwingEngine`, sets SL just behind the nearest confirmed swing low (long) / high (short) via the existing broker-clamped never-loosen `_modify_sl`. Entry already supplies the structural SL + liquidity TP; TP1 partial (33% @ `PARTIAL_TRIGGER_RR`) + early BE (@ `BE_TRIGGER_RR=0.3`, stricter than textbook "BE after TP1" for FTMO safety) reuse the existing skeleton. TF `_exit_profile` branch removed. ⚠️ v1 = single structural partial + structure-trailed runner to the liquidity TP; explicit TP2/TP3 intermediate partials are a deferred refinement. `import main`+mock-init clean, SMC tests 9/9.

**Next (P5–P7)**: `ml/smc_backtester.py` (`SMCBacktester(StrategyBacktester)`) + `ml/smc_env.py` (`SMCFilterEnv(FTMOSignalFilterEnv)`, mirror smc_v1 24-dim `_get_obs`) + SMC pool/GBM/RL training (needs D1/M5 fetched) + DELETE MR core + repoint `signal_filter_env`/audits → validation (leakage/parity/obs_parity exit 0, walk-forward, beat-old-SMC gate) → docs.

**⚠️ Status**: `main` (MR production) UNCHANGED = rollback. SMC rebuild lives ONLY on `rebuild/smc-v2`; NOT deployed, NOT committed yet. Docs `01-architecture`/`02-modules` (MR rows)/`03-rl-training`/`04-operations` still describe the MR system — they get rewritten when MR source is removed in P3.
