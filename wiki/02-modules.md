...
> Last Updated: 2026-06-06 (🔧 rebuild/smc-v2 — NEW strategy/smc/ engine package; MR rows below describe `main`) | Scope: every module + key class / method / variable
>
> **🔧 rebuild/smc-v2 (2026-06-06) — NEW `strategy/smc/` engine package (Brain 1 = SMC rules)**. Live on branch `rebuild/smc-v2` only; `main` still = MR. Full rationale → [`05-invariants.md` SMC v2 kickoff](05-invariants.md#-version-log-entry--smc-v2-rebuild-kickoff-2026-06-06-branch-rebuildsmc-v2-p0p2-done):
> - `strategy/smc/types.py` — value objects `Direction`, `SwingPoint`, `StructureEvent`, `FVG`, `OrderBlock`, `LiquidityLevel`, `LiquiditySweep`, `DealingRange` + enums `SwingType/SwingLabel/StructureKind/Zone`. ⛔ `SwingPoint.confirm_index` = no-lookahead gate (a swing is usable only from that bar).
> - `strategy/smc/swing.py` — `SwingEngine` (fractal swing detect, ATR noise filter, internal vs external, HH/HL/LH/LL labels) + `_atr` helper
> - `strategy/smc/structure.py` — `StructureEngine` / `StructureState` (BOS = continuation, CHOCH = reversal; body-close confirmed)
> - `strategy/smc/liquidity.py` — `LiquidityEngine` (equal H/L pools, liquidity sweeps, `dealing_range` → premium/discount/OTE)
> - `strategy/smc/fvg.py` — `FVGEngine` (3-candle imbalance + mitigation states)
> - `strategy/smc/order_block.py` — `OrderBlockEngine` (last opposing candle before a BOS + quality grade + mitigation)
> - `strategy/smc/bias.py` — `BiasEngine` / `MTFBias` (HTF top-down bias + next liquidity target)
> - `strategy/smc/entry.py` — `EntryEngine` + `SMCEntryConfig` + `EntryResult`: the STRICT sequential state machine (HTF bias → discount/premium → LTF sweep → CHOCH/BOS → unmitigated OB/FVG zone → structural SL/TP + RR gate). `SMCSignal` + `SMCSignalType` (BUY/SELL) live in `strategy/smc/types.py`.
> - `strategy/smc_strategy.py` — `SMCScanner(StrategyBase)` (STRATEGY_ID="SMC", MAGIC 123458, OBS_LAYOUT_ID="smc_v1"): per-symbol caches, fetches D1(HTF)/H4(MTF)/M15(LTF) via connector, runs `EntryEngine`, returns `List[SMCSignal]`. `pip_size_for(symbol)` helper.
> - tests: `tests/test_smc_engines.py` (**9 passing** — incl. end-to-end entry state machine). `scripts/fetch_mt5_data.py` extended with D1 + M5 timeframes.
> - ✅ P3 DONE: `main.py` rewritten to SMC — `FTMOTradingBot` builds `SMCScanner` (magic 123458), RL←`models/smc/`, GBM←`data/smc_signal_quality_model.pkl`, `_build_signal_observation` = **smc_v1 24-dim**. ⛔ `SelfLearningAgent.OBS_DIM` 36→**24**. Removed: dual router, per-strategy dicts, dead regime methods, Chronos (`ml/strategy_backtester.py` chronos block too). **DELETED**: `strategy/trend_following_strategy.py`, `ml/trend_following_backtester.py`, `ml/trend_following_env.py`, `strategy/regime_classifier.py`, `strategy/strategy_router.py`, `ml/chronos_forecaster.py`, `scripts/{build_tf_signal_pool,train_tf_signal_quality,train_tf_signal_filter,auto_train_pipeline_tf,diagnose_chronos}.py`.
> - ✅ P4 DONE: `execution/trade_manager.py` — `_exit_profile` is SMC (`trail_mode="structure"`, partial + early BE, no tp_step); NEW `TradeManager._structure_trail` trails SL behind the latest M15 swing (via `SwingEngine` + the broker-clamped never-loosen `_modify_sl`). TF exit branch + Chronos block (in `ml/strategy_backtester.py`) removed. `STRUCTURE_TRAIL_BUFFER_ATR=0.1`.
> - ⏳ PENDING: (P5) `ml/smc_backtester.py` + `ml/smc_env.py` + training pipeline + DELETE MR core (`mean_reversion_strategy/backtester/env`, MR scripts) + repoint `signal_filter_env`/audits. MR modules below STILL PRESENT on this branch until P5 (base env + audits import them).
>
...
| `indicators.py` | `TechnicalIndicators.atr`, `.ema`, `.rsi`, `.macd`, `.adx`, `.stoch`, `.bb`, **`.calculate_choppiness` (v8.1, static, 0-100)**, **`.calculate_signed_efficiency` (Fix#1 2026-06-03, static, [−1,+1])** | Indicator calculation on numpy arrays. **2026-06-02 (TF early-entry S1)**: `calculate_adx` also persists `df['plus_di']`/`['minus_di']`/`['di_spread']` (additive, `adx` byte-identical). **Fix#1 2026-06-03**: `calculate_signed_efficiency(df, period)` = signed Kaufman ER `(close[-1]−close[-1-p])/Σ|Δclose|` → persistent-trend regime detector (powers obs[35]=`trend_eff_h1`, H1 period 24). |
...
| `mean_reversion_backtester.py` (v8.0) | `MeanReversionBacktester(StrategyBacktester)`, `.generate_episode_signals`, `._bars_to_first_hit`, `MR_SCAN_POINTS_PER_DAY=48` (v8.0.2 = every 30min), `MR_FUTURE_BARS=32`, `DEDUP_BARS=4` | Pool builder for MR. Scans every 30min (48/day) — BB extremes are time-sensitive. `DEDUP_BARS=4` prevents same-direction signal flooding. Adds `bars_to_resolution` + `is_quick_tp` per signal for env reward shaping. Pool path: `data/mr_signal_pool_<N>.pkl`. **Fix#1 (2026-06-03)**: sig_dict adds `trend_eff_h1` (= `calculate_signed_efficiency(h1_slice,24)`) → obs[35]. **Fix#3 (2026-06-04 — sim↔live realism, PROMOTED)**: every `outcome_pnl_ratio` now **deducts a fixed round-trip spread cost** (`_TYPICAL_SPREAD_PIPS / sl_distance_pips` in R) — the legacy ±0.2% multiplicative slippage didn't model the fixed bid-ask cost that eats ~10% of a tight RR≈1.0 trade (the live payoff-0.59 gap). `model_live_exit=True` resolves via `_resolve_trade_live_exit` (BE@0.3R+partial@0.8R) instead of full ride. |
...
| Script | Role |
|--------|------|
| **`build_mr_signal_pool.py`** (v8.0) | Multiprocessing 8 workers — calls `MeanReversionBacktester.generate_episode_signals` × N → `data/mr_signal_pool_<N>.pkl`. Replaces legacy `build_signal_pool.py` (deleted with SMC in v8.0.6). **2026-06-03**: added `--start_frac/--end_frac` (chronological M15-bar window for walk-forward folds; backward-compatible — defaults `0.0/1.0` = full history byte-for-byte). |
...
| **`holdout_eval.py`** (v8.0.10) | **Anti-overfit verification** — runs the saved model (`models/mr/best/ppo_mr_filter.zip` + `vec_normalize_mr.pkl`) on TWO independently-seeded MR pools (training pool seed=42 vs holdout pool seed=999) for `--n_episodes` each, prints Pass / Win / DD / Profit metrics per pool, then computes Δ Pass Rate. Verdict: ≤ 5 pp = HEALTHY, 5-10 pp = MILD, > 10 pp = OVERFIT (exit 1). Holdout pool built via `build_mr_signal_pool.py --seed 999 --save_path data/mr_signal_pool_holdout.pkl`. ⚠️ Limitation: both pools are the **same OHLCV history** resampled by seed → cannot detect regime/time shift (use `walk_forward_eval.py` for that). |
| **`walk_forward_eval.py`** (2026-06-03, rebuild/regime-aware) | **Out-of-TIME generalization** — splits OHLCV by **time** (M15 bar-index is chronological) into folds `older`/`mid`/`recent` via `build_mr_signal_pool --start_frac/--end_frac`, evals the deployed model per fold, prints Pass/Win/Take/Profit/EV-per-trade + an `older − recent` verdict. **First real run (n=600/fold): Pass 81/78/90% = STABLE → model NOT time-overfit in sim** (the FTMO fail is the sim↔live execution gap). `--model_live_exit` rebuilds fold pools with the LIVE exit (BE+partial) instead of "ride" → quantifies the exit-realization slice of that gap. Mode 2 (retrain-on-older → test-recent = true OOS) is the next gate. Run: `walk_forward_eval.py --pool_size 600 --n_episodes 800 --workers 8 [--model_live_exit]`. |
