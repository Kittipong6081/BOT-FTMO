...
> Last Updated: 2026-06-04 (✅ Fix#1 obs 36; ✅ Fix#3 — backtester ALWAYS-ON spread cost + train-with-live-exit, best/ = realistic model) | Scope: RL env, obs space, reward shaping (MR), PPO hyperparams, curriculum, aux task.
>
...
>
> **✅ Fix#3 (2026-06-04) — `model_live_exit` PROMOTED for training + ALWAYS-ON spread cost**: the deployed `models/mr/best/` is now trained on a **realistic** pool. (1) `MeanReversionBacktester.generate_episode_signals` ALWAYS deducts a fixed round-trip spread cost (`typical_spread_pips/sl_distance_pips` R) from every outcome (the legacy ±0.2% multiplicative slippage missed the fixed bid-ask cost = the live payoff-0.59 gap). (2) `auto_train_pipeline --model_live_exit` (forces rebuild) builds the pool via `_resolve_trade_live_exit` (BE@0.3R + 33% partial@0.8R) instead of "ride". **Honest result: Pass 21% (not the optimistic 87%); verify shows the realistic-trained model passes FTMO 6× more than the ride-trained one (17.3% vs 2.8%).** ⚠️ pool outcomes are now net-of-cost — any new pool/holdout/walk-forward reflects this. Earlier the experiment-era note claimed "ride EV better"; under the full FTMO objective (Pass-rate, not per-trade EV) the realistic model wins. See [`05-invariants.md` Fix#3](05-invariants.md#-version-log-entry--fix3-sim-realism-spread--live-exit-training-2026-06-04-rebuildregime-aware) + memory `project_exit_machinery_ev_leak`.
...
**🔧 Fix#3 — sim↔live realism (2026-06-04, RETRAIN IN PROGRESS, not yet verified/promoted)**: the walk-forward proved the model is NOT time-overfit (stable across folds) — so the live failure is the **sim↔live execution gap**, not overfit. `model_live_exit` (live-exit walk-forward) showed the backtester's full-position "ride" exits overstate Pass ~88%→~54% vs the realistic BE/partial exits live uses. Fix#3 makes the **pool itself** realistic so the agent trains on what live realizes: (1) `MeanReversionBacktester` now **deducts a fixed round-trip spread cost** from every `outcome_pnl_ratio` (always on); (2) `auto_train_pipeline --model_live_exit` builds the pool with BE@0.3R+partial@0.8R exits (forces rebuild). Run: `auto_train_pipeline.py --model_live_exit --pool_size 5000 --outcome_noise 0.08 --risk_per_trade 0.0070 ...`. Expected: Pass drops toward the honest ~50-60% (the model becomes more selective — skips marginal trades that are net-negative after cost). Gate before promote: walk-forward (same realistic settings) ≥ Fix#1 baseline. See `~/.claude/plans/hedge-fund-quant-flickering-pudding.md`.
